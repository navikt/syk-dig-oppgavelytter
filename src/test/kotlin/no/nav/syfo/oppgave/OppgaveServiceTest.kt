...
class OppgaveServiceTest :
    FunSpec({
        ...
        context("OppgaveService") {
            test("Henter dokumentInfoId for utenlandsk sykmelding-oppgave som kommer fra Rina") {
                ...
                coVerify {
                    oppgaveClient.oppdaterOppgave(
                        match {
                            it.id == 1 && it.versjon == 1 && it.behandlesAvApplikasjon == "SMD"
                        },
                        any()
                    )
                }
            ...
            test(
                "Henter ikke dokumentInfoId for utenlandsk sykmelding-oppgave med behandlesAvApplikasjon SMD"
            ) {
                coEvery { oppgaveClient.hentOppgave(any(), any()) } returns
                    OppgaveResponse(
                        journalpostId = "5566",
            ...
            test(
                "Henter ikke dokumentInfoId for ferdigstilt utenlandsk sykmelding-oppgave som kommer fra Rina"
            ) {
                coEvery { oppgaveClient.hentOppgave(any(), any()) } returns
                    OppgaveResponse(
                        journalpostId = "5566",
